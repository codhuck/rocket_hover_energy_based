...
This constant $C_{m\alpha}$ is the central quantity that the adaptive controller estimates online via Certainty Equivalence. Only $C_{m\alpha}$ appears in the rotational dynamics — this is the parameter targeted by the adaptive law. The translational coefficients $C_x$ and $C_{y\alpha}$ affect $(x, y)$ motion only and lie outside the scope of the angular control loop.

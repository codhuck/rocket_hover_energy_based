...
def _targets(result: SimulationResult) -> tuple[float, float]:
    ...
    h_target = float(mission.get("h_target", mission.get("htarget", 0.0)))
    return x_land, h_target
...
def _mission_markers(result: SimulationResult):
    ...
    return [
        ...
        ("LAND", x_land, 0.0, "X", PALETTE["ground_line"]),
    ]
...
def _draw_mission_markers(ax, markers):
    ...
    xs = [point[1] for point in markers]
    ys = [point[2] for point in markers]
...
def _mission_view_limits(result: SimulationResult, mission_markers):
    ...
    marker_x = [point[1] for point in mission_markers]
    marker_y = [point[2] for point in mission_markers]
...
def draw_rocket(
    ...
    for pts, fc, ec, lw, zorder in rocket_parts:
        ax.add_patch(
            mpatches.Polygon(
    ...
    flame_scale = max(float(throttle), 0.02)

    flame_layers = [
        (3.2 * flame_scale, 0.40, PALETTE["flame_outer"]),
        (2.2 * flame_scale, 0.27, PALETTE["flame_mid"]),
        (1.1 * flame_scale, 0.14, PALETTE["flame_core"]),
    ]

    for length, width, color in flame_layers:
        flame_pts = np.array(
            [
                [-width, 0],
                [width, 0],
...
def save_all_figures(result: SimulationResult, output_dir: Path) -> None:
    ...
    ax.scatter([x_land], [0], marker="X", s=90, color=PALETTE["ground_line"], zorder=5, label="landing target")

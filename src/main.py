...
def _run_single(cfg: dict, output_root: Path, animate: bool = False) -> None:
    ...
    if animate:
        save_animation(result, animations_dir / "mpc_mission.gif")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run planar TVC rocket MPC mission simulation.")
    parser.add_argument("--config", type=Path, default=Path("configs/mpc.yaml"))
    parser.add_argument("--output-root", type=Path, default=Path("outputs/mpc"))
    parser.add_argument("--animate", action="store_true", default=False, help="Also render a GIF animation.")
